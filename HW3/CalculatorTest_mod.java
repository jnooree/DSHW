...
class PostFixer
{
	...
	public static List<Terms> convert(String inExpr) throws IllegalArgumentException
	{
		...
		ParChecker(inExpr);
		List<Terms> inFixList = split(inExpr);
	...
	private static void ParChecker(String inExpr) throws IllegalArgumentException
	{
		int parCounts = 0;

		for(int i=0; i<inExpr.length(); i++)
		{
			if(inExpr.charAt(i) == '(') ++parCounts;
			else if(inExpr.charAt(i) == ')') --parCounts;
		}

		if(parCounts != 0) throw new IllegalArgumentException();
	}

	private static List<Terms> split(String inExpr) throws IllegalArgumentException
	{
		...
		if(inExpr.charAt(0) == '-')
		{
			...
			result.addAll(split(inExpr.substring(1)));
		}
		...
		{
			result.addAll(rxInExpr.getOper(parCounts));
			result.addAll(split(rxInExpr.getInExpr(), 0));
		}
	...
	private static int rParFinder(String inExpr)
	{
		for(int i=0; i<inExpr.length(); i++)
		{
			if(inExpr.charAt(i) == ')') return i;
	...
	{
		private final Pattern NUMR_PATTERN = Pattern.compile("^(\\d+)");
		private final Pattern OPER_PATTERN = Pattern.compile("^(.*?[^\\^\\+\\-\\*\\/\\%]+)([\\^\\+\\-\\*\\/\\%])(.+)");
		...
		List<Terms> getOper(int parCounts)
		{
			List<Terms> result = new ArrayList<>();
			...
			result.addAll(split(inExpr, parCounts));
